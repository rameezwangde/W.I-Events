...
export function Header() {
  ...
  return <header className={`header ${scrolled ? 'is-scrolled' : ''}`}>
    <Link to="/" className="brand" aria-label="Natak Baaz Home"><img src="/images/logo.png?v=2" alt="Natak Baaz" className="header__logo-img" style={{ maxHeight: '120px', width: 'auto' }} /></Link>
...
export function Footer() {
  return <footer className="footer" role="contentinfo">
    ...
    <div className="footer__grid">
      <div>
        <Link to="/" aria-label="Natak Baaz Home">
          <img src="/images/logo.png?v=2" alt="Natak Baaz - The Nukkad Natak Group" className="footer__logo-img" style={{ maxHeight: '130px', marginBottom: '1.5rem', width: 'auto' }} />
        </Link>
    ...
    <div className="footer__bottom">
      <span>© {new Date().getFullYear()} Natak Baaz · All Rights Reserved</span>
      <span><Link to="/privacy">Privacy</Link><Link to="/terms">Terms</Link><a href="/sitemap.xml">Sitemap</a></span>
    </div>
  </footer>
}
...
export function Gallery({ full = false }) {
  const cats = ['All', 'CSR Activities', 'Roadshows', 'Social Awareness', 'Rural Marketing', 'Theatre']; const [cat, setCat] = useState('All'); const [light, setLight] = useState(null); const items = Array.from({ length: full ? 10 : 6 }, (_, i) => ({ cat: cats[(i % 5) + 1], img: i % 3 === 1 ? '/images/street-theatre-rehearsal-delhi.webp' : '/images/nukkad-natak-delhi-performance.webp', cap: ['Live street theatre performance', 'Rehearsal and ensemble preparation', 'Audience-focused public outreach'][i % 3] })); const shown = cat === 'All' ? items : items.filter(x => x.cat === cat)
